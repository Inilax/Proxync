...
export function WelcomeView({
  ...
  const getTrafficStats = (tunnelId: string) => {
    ...
    if (isCurrentActive) {
      const uploadBytes = requests.length * 1536; // 1.5 KB per request
      const downloadBytes = requests.length * 8602; // 8.4 KB per request
      ...
      <section className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-surface-container-low p-8 rounded-xl border border-outline-variant relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-on-surface-variant max-w-md">
            Proxync is currently monitoring {activeTunnels.length} active {activeTunnels.length === 1 ? 'tunnel' : 'tunnels'} across us-east-1 relay nodes.
          </p>
        </div>
        <button
          onClick={onDiscover}
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-xs text-outline mt-0.5">
                  Click here or "Create New Tunnel" to expose a local process
                </p>
              </div>
            </div>
          ) : (
                      ...
                      {activeMenuTunnelId === tunnel.id && (
                        <>
                          ...
                          <div className="absolute right-0 mt-1 w-44 rounded-lg bg-surface-container border border-outline-variant shadow-xl py-1 z-50 animate-scale-in">
                            ...
                            <button
                              disabled
                              className="flex items-center gap-2 px-4 py-2 w-full text-left text-xs text-on-surface/40 cursor-not-allowed opacity-50"
                            >
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="btn-secondary flex-1"
                >
                  <span className="font-label-md text-label-md block text-on-surface">LAN Sharing</span>
                </button>
              </div>
            </div>
          </div>
